...
def test_browser_bridge_round_trip() -> None:
    ...
    def requester() -> None:
        result.update(bridge.request("gmail_read_inbox", {"max_messages": 20}, timeout=1))

    thread = threading.Thread(target=requester)
    thread.start()
    command = bridge.next_command(timeout=1)
    assert command is not None
    assert bridge.status() == {"connected": True, "pending": 1}
    assert command["action"] == "gmail_read_inbox"
    assert command["payload"] == {"max_messages": 20}
    assert bridge.resolve(command["id"], success=True, data={"messages": []})
    thread.join(timeout=1)

    assert result == {"messages": []}
    assert bridge.status() == {"connected": True, "pending": 0}
...
def test_browser_bridge_times_out_when_extension_is_not_connected() -> None:
    ...
    with pytest.raises(BrowserBridgeError, match="main Brave profile"):
        bridge.request("gmail_read_inbox", timeout=0.01)
...
def test_browser_bridge_surfaces_extension_error() -> None:
    ...
    def requester() -> None:
        try:
            bridge.request("gmail_read_inbox", timeout=1)
